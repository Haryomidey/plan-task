const todaysDayEl = document.querySelector<HTMLHeadingElement>('.todays-day');
const todaysDateEl = document.querySelector<HTMLHeadingElement>('.todays-date');
const todaysMonthEl = document.querySelector<HTMLHeadingElement>('.calendar-container calendar h3');
const calendarEl = document.querySelector<HTMLDivElement>('.calendar');

const taskForm = document.getElementById('task-form') as HTMLFormElement;
const taskTitleEl = document.getElementById('task-title') as HTMLInputElement;
const taskDescEl = document.getElementById('task-desc') as HTMLTextAreaElement;
const taskTimeEl = document.getElementById('task-time') as HTMLInputElement;

const taskContainer = document.querySelector<HTMLDivElement>('.tasks');

const totalTasksEl = document.querySelector<HTMLParagraphElement>('.summary-card.total p');
const pendingTasksEl = document.querySelector<HTMLParagraphElement>('.summary-card.pending p');
const completedTasksEl = document.querySelector<HTMLParagraphElement>('.summary-card.completed p');
...
let tasks: Tasks[] = JSON.parse(localStorage.getItem('tasks') || '[]');
...
const createTask = (taskTitle: string, taskDesc: string, taskTime: string) => {
    if (!taskTitleEl || !taskDescEl || !taskTimeEl) return;
    const newTask: Tasks = {
        taskTitle,
        taskDesc,
...
const attachTaskEvents = () => {
    if (!taskContainer) return;
    const taskEls = taskContainer.querySelectorAll<HTMLDivElement>('.task');

    taskEls.forEach((taskEl) => {
        const index = parseInt(taskEl.dataset.id || '0');

        const completeBtn = taskEl.querySelector<HTMLButtonElement>('.btn-complete');
        ...
        const deleteBtn = taskEl.querySelector<HTMLButtonElement>('.btn-delete');
        ...
        const editBtn = taskEl.querySelector<HTMLButtonElement>('.btn-edit');
...
const updateSummary = () => {
    ...
    if (pendingTasksEl) pendingTasksEl.textContent = pending.toString();
    if (completedTasksEl) completedTasksEl.textContent = completed.toString();
};

const today = new Date();

const renderCalendar = (year: number, month: number): string => {
